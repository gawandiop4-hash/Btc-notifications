...
const CHART_API = 'https://api.binance.com/api/v3/klines?symbol=BTCUSDT&interval=1h&limit=555';
...
const VP_PROXIMITY_THRESHOLD = 0.02;
...
console.log('Wake Lock rilasciato, tento di riacquisirlo');
...
console.log('Service Worker registrato:', swRegistration.scope);
} catch (e) {
console.log('Errore registrazione SW:', e);
swRegistration = null;
}
}
...
console.log('App in background - continuo monitoraggio RSI');
...
console.log('Avvio monitoraggio background');

backgroundCheckInterval = setInterval(async function() {
try {
await fetchRSIData();
console.log('Background check - RSI:', rsiValue);
} catch (e) {
console.error('Errore background check:', e);
}
}, 30000);
}
...
console.log('Stop monitoraggio background');
}
}
...
if (swRegistration) {
swRegistration.showNotification('Sistema Notifiche Attivo', options)
.then(function() { console.log('Notifica test inviata via SW'); })
.catch(function(e) { console.error('Errore SW notification:', e); });
} else {
var n = new Notification('Sistema Notifiche Attivo', options);
...
console.error('Errore playAlertSound:', e);
}
}
...
console.log('Tentativo notifica RSI:', level, 'Permission:', Notification.permission);

if (!('Notification' in window) || Notification.permission !== 'granted') {
console.log('Notifiche non permesse. Stato:', Notification ? Notification.permission : 'Non supportato');
return;
}
...
var notificationOptions = {
  ...
  timestamp: Date.now(),
  data: { level: level, time: new Date().toISOString() }
};

if (swRegistration) {
  swRegistration.showNotification(title, notificationOptions)
    .then(function() {
      console.log('Notifica RSI inviata via SW (lock screen)');
    })
    .catch(function(e) {
      console.error('Errore SW showNotification:', e);
      currentNotification = new Notification(title, notificationOptions);
    });
} else {
  currentNotification = new Notification(title, notificationOptions);
  ...
  currentNotification.onerror = function(e) { console.error('Errore notifica:', e); };
  currentNotification.onshow = function() { console.log('Notifica mostrata'); };
}
...
console.log('RSI Alert triggered! Level:', level, 'Current RSI:', rsiValue);
showRSIAlert(level);
break;
}
}
...
'''
  ...
  console.log('RSI aggiornato:', rsiValue);
